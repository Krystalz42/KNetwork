...
namespace KNW {
	...
	public:
		...
		void startServer(std::string port) noexcept;
	...
	void ServerTCP::writeDataToOpenConnection(T data, int index) {
		...
		connections[index]->write(
				serializeData(DataType<T>::getHeader(), data));
	}
	...
	void ServerTCP::writeDataToOpenConnection(T data, int index, H header) {
		...
		connections[index]->write(serializeData(header, data));
	}
	...
	void ServerTCP::writeDataToOpenConnections(T data, H header) {
		...
		for (auto &connection : connections) {
			if (connection)
				connection->write(dataTCP->serializeData(header, data));
		}
	}
